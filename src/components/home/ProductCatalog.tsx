...
export default function ProductCatalogAlt({ content }: ProductCatalogProps) {
    ...
    try {
      ...
      const transformedProducts = (response.payload || []).map((product: any) => {
        ...
        return {
          ...
          id: product._id,
          name: product.title,
          images: product.thumbnails?.[0] ? [product.thumbnails[0]] : ["/placeholder.svg"],
    ...
    addToCart(
      {
        ...
        price: product.price,
        image: product.images?.[0] || product.thumbnails?.[0] || "/placeholder.svg",
  ...
  if (loading && products.length === 0) {
    ...
    return (
      <section id="products" className="py-12 lg:py-32" style={{ backgroundColor: "var(--creme)" }}>
                ...
                <div className="mb-4 md:mb-6 mt-auto">
                  ...
                  <div className="flex items-baseline space-x-2 mb-1">
                    {product.originalPrice && (
                      <span className="text-sm md:text-base line-through opacity-60" style={{ color: "var(--oak)" }}>
                        ${product.originalPrice.toLocaleString()}
                      </span>
                    )}
                    <span className="text-lg md:text-2xl font-semibold" style={{ color: "var(--deep-clay)" }}>
                      ${product.price.toLocaleString()}
                    </span>
                  </div>
                  {product.originalPrice && (
                    <div className="text-xs md:text-sm font-medium" style={{ color: "var(--accent-gold)" }}>
